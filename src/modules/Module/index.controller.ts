...
import moduleService from "../Module/index.service";
...
moduleController.get("/modules/:subjectId", async (req: Request, res: Response) => {
...
moduleController.get("/modules/:id", async (req: Request, res: Response) => {
  // #swagger.tags = ['Modules']
  try {
    const token = req.headers.authorization;
    if (!token) throw Error("token field empty");
...
moduleController.delete("/modules/:id", async (req: Request, res: Response) => {
  // #swagger.tags = ['Modules']
  try {
    const token = req.headers.authorization;
    if (!token) throw Error("token field empty");
...
moduleController.patch("/modules/:id", async (req: Request, res: Response) => {
  // #swagger.tags = ['Modules']
  try {
    const token = req.headers.authorization;
    if (!token) throw Error("token field empty");
...
moduleController.post("/modules", async (req: Request, res: Response) => {
  // #swagger.tags = ['Modules']
